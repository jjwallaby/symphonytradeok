...
var helloControllerService = SYMPHONY.services.register("tradeokblt:controller");
...
SYMPHONY.remote.hello().then(function(data) {
    ...
    SYMPHONY.application.register("tradeokblt", ["modules", "applications-nav", "ui", "share", "entity"], ["tradeokblt:controller", "message:controller"]).then(function(response) {
        ...
        //"http://rugbyfc.com/symphonyimg/tradeok.png"
        // LEFT NAV: Add an entry to the left navigation for our application
        navService.add("tradeokblt-nav", { title : "Trade OK", icon: "/img/tradeok.png"}, "tradeokblt:controller");

        // LEFT NAV: Add an extra left navigation item that can be removed by clicking on the "Remove Left Nav Item Button"
        //navService.add("tradeokblt-nav-remove", "Removable Left Nav Item", "tradeokblt:controller");
        ...
        uiService.registerExtension("single-user-im", "tradeokblt-im", "tradeokblt:controller", {label: "IM Button", data: {"datetime": Date()}});
        uiService.registerExtension("multi-user-im", "tradeokblt-mim", "tradeokblt:controller", {label: "MIM Button"});
        uiService.registerExtension("room", "tradeokblt-room", "tradeokblt:controller", {label: "Room Button"});
        uiService.registerExtension("profile", "tradeokblt-profile", "tradeokblt:controller", {label: "Profile Button"});
        uiService.registerExtension("hashtag", "tradeokblt-hashtag", "tradeokblt:controller", {label: "Hashtag Link"});
        uiService.registerExtension("cashtag", "tradeokblt-cashtag", "tradeokblt:controller", {label: "Cashtag Link"});
        uiService.registerExtension("settings", "tradeokblt-settings", "tradeokblt:controller", {label: "Settings Link"});

        // SHARE: Set the controller that implements the "link" method invoked when shared articles are clicked on.
        shareService.handleLink("article", "tradeokblt:controller");
        ...
        helloControllerService.implement({
            ...
            select: function(id) {
                if (id == "tradeokblt-nav") {
                    // Focus the left navigation item when clicked
                    navService.focus("tradeokblt-nav");
                }

                modulesService.show("tradeokblt", {title: "Trade Blotter" ,icon: "http://rugbyfc.com/symphonyimg/tradeok.png" }, "tradeokblt:controller", "https://localhost:4000/app.html", {
                    // You must specify canFloat in the module options so that the module can be pinned
                    "canFloat": true,
                });
                // Focus the module after it is shown
                modulesService.focus("tradeokblt");
            },
            ...
            trigger: function(uiClass, id, payload, data) {
                if (uiClass == "single-user-im") {
                    ...
                    var moduleTitle = "TradeOK App: " + cashtag;
                    modulesService.show("tradeokblt-cashtag", {title: moduleTitle}, "tradeokblt:controller", "https://localhost:4000/app.html?cashtag=" + cashtag, {
                        "canFloat": true,
                        // Use parentModuleId to open a new module without closing the original module ("tradeokblt")
                        "parentModuleId": "tradeokblt"
                    });
                    modulesService.focus("tradeokblt-cashtag");
            ...
            link: function(type, articleId) {
                if(type == "article") {
                    var moduleTitle = "TradeOKBlt App: " + articleId;
                    modulesService.show("tradeokblt-article", {title: moduleTitle}, "tradeokblt:controller", "https://localhost:4000/app.html?article=" + articleId, {
                        "canFloat": true,
                        // Use parentModuleId to open a new module without closing the original module ("tradeokblt")
                        "parentModuleId": "tradeokblt"
                    });
                    modulesService.focus("tradeokblt-article");
                }
            },
        });
